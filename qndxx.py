...
class QnDxx(object):
    ...
    def get_course(self) -> Tuple[int, str]:
        ...
        获取最新课程id和标题
        '''
        res = self.session.get(url=self.bjyouth_dxx_index_url)
        res_json = res.json()
    ...
    def get_org_id(self) -> int:
        ...
        获取当前组织ID
        '''
        res = self.session.get(url=self.bjyouth_dxx_league_url)
        org_id = int(res.text)
        return org_id
